 ...
 * Returns the outcome (result + error) so callers can distinguish
 * success-with-no-output from actual failures.
 */
    ...
    const output = await runContainerAgent(
      ...
      async (streamedOutput: ContainerOutput) => {
        ...
        if (streamedOutput.status === 'success') {
          error = null; // Clear any transient error — agent recovered
