...
export async function handleSessionCommand(opts: {
  ...
  // /compact: forward the literal slash command as the prompt (no XML formatting)
...
async function handleClear(
  ...
  try {
    ...
    deps.advanceCursor(cmdMsg.timestamp);
    await deps.setTyping(false);
    await deps.sendMessage('Session cleared. Next message starts a fresh conversation.');
