...
/** IPC handler that mirrors the real handler. */
...
describe('StreamToolLogger pipeline: CEO agent tool call capture', () => {
  ...
  it('captures multiple tool calls from a single ceo stream session', () => {
    ...
    logger.processLine(
      JSON.stringify({ type: 'tool_use', id: 'toolu_1', name: 'Bash', input: { command: 'git status' } }),
    );
    logger.processLine(
      JSON.stringify({ type: 'tool_result', tool_use_id: 'toolu_1', content: 'On branch main' }),
    );

    logger.processLine(
      JSON.stringify({ type: 'tool_use', id: 'toolu_2', name: 'Read', input: { file_path: '/workspace/CLAUDE.md' } }),
    );
    logger.processLine(
      JSON.stringify({ type: 'tool_result', tool_use_id: 'toolu_2', content: [{ type: 'text', text: '# Project README' }] }),
    );

    logger.processLine(
      JSON.stringify({ type: 'tool_use', id: 'toolu_3', name: 'Edit', input: { file_path: '/workspace/test.ts', old_string: 'foo', new_string: 'bar' } }),
    );
    logger.processLine(
      JSON.stringify({ type: 'tool_result', tool_use_id: 'toolu_3', content: 'Edit applied successfully' }),
    );
