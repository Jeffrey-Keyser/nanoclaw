...
vi.mock('child_process', () => {
  ...
  return {
    spawn: vi.fn(() => {
      ...
      // Auto-close with exit code 0 by default
...
describe('crash-handler', () => {
  ...
  describe('formatCrashNotification', () => {
    ...
    it('formats failed restart notification with follow-up mention', () => {
      const event: CrashEvent = {
    ...
    it('truncates long error summaries', () => {
      ...
      const msg = formatCrashNotification(event);
      // Error summary in the notification is capped at 800 chars
  ...
  describe('handleServiceCrash', () => {
    it('runs full workflow and sends notification on successful restart', async () => {
      // Mock spawn to return journal logs then successful restart
      ...
      spawnMock.mockImplementation(() => {
        ...
        setTimeout(() => {
          ...
          // restart call (callCount === 2) — just succeed
          child.emit('close', 0);
        }, 0);
        return child as any;
      });
