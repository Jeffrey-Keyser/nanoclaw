...
describe('extractSessionCommand', () => {
  ...
  it('rejects /clear with trailing text', () => {
    expect(extractSessionCommand('/clear everything', trigger)).toBeNull();
  });

  it('handles /clear with whitespace', () => {
    expect(extractSessionCommand('  /clear  ', trigger)).toBe('/clear');
  });
...
describe('handleSessionCommand', () => {
  ...
  it('silently ignores unauthorized /clear in non-main, not-from-me', async () => {
    const deps = makeDeps();
    const result = await handleSessionCommand({
    ...
    expect(deps.deleteInMemorySession).not.toHaveBeenCalled();
  });

  it('allows is_from_me /clear in non-main group', async () => {
    const deps = makeDeps();
    const result = await handleSessionCommand({
  ...
  it('processes pre-clear messages before clearing', async () => {
    const deps = makeDeps();
    const msgs = [
    ...
    expect(result).toEqual({ handled: true, success: true });
    expect(deps.formatMessages).toHaveBeenCalledWith([msgs[0]], 'UTC');
    // Two runAgent calls: pre-clear messages + /clear archive
    expect(deps.runAgent).toHaveBeenCalledTimes(2);
    expect(deps.runAgent).toHaveBeenCalledWith('<formatted>', expect.any(Function));
    expect(deps.runAgent).toHaveBeenCalledWith('/clear', expect.any(Function));
    expect(deps.deleteSession).toHaveBeenCalledWith('test-group');
  ...
  it('does not delete session on /clear archive failure', async () => {
    const deps = makeDeps({
      runAgent: vi.fn().mockImplementation(async (_prompt, onOutput) => {
        await onOutput({ status: 'error', result: 'archive failed' });
        return 'error';
      }),
    });
    const result = await handleSessionCommand({
    ...
    expect(deps.deleteSession).not.toHaveBeenCalled();
    expect(deps.deleteInMemorySession).not.toHaveBeenCalled();
    expect(deps.sendMessage).toHaveBeenCalledWith('/clear failed. The session is unchanged.');
    expect(deps.advanceCursor).not.toHaveBeenCalled();
  });

  it('does not advance cursor on /clear failure', async () => {
    const deps = makeDeps({
      runAgent: vi.fn().mockImplementation(async (_prompt, onOutput) => {
        await onOutput({ status: 'error' });
        return 'error';
      }),
    });
    const result = await handleSessionCommand({
