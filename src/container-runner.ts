...
async function buildContainerArgs(
  ...
  // Optional opt-in: when set, the container's activity-events helper
  // broadcasts tool_call_start + decision events as chat messages on the
  // session's default routing. Default off (no env var pushed). Read in
  // container/agent-runner/src/activity-events.ts.
  if (ACTIVITY_AUTO_STREAM) {
    args.push('-e', 'ACTIVITY_AUTO_STREAM=true');
  }
