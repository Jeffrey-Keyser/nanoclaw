...
_fetch_secret() {
  ...
  # Support both {"value":"..."} and raw string responses
  echo "$response" | jq -re '.value // .' 2>/dev/null || echo "$response"
}
...
_pay_resp=$(curl -sf -X POST "http://host.docker.internal:3017/auth/login" \
  -H "Content-Type: application/json" \
  -d "{\"email\":$(printf '%s' "$_pay_email" | jq -Rs .),\"password\":$(printf '%s' "$_pay_password" | jq -Rs .)}" \
  2>/dev/null) || { _warn "Pay login request failed — AGENT_PAY_TOKEN not set"; _pay_resp=""; }

if [[ -n "$_pay_resp" ]]; then
  _token=$(echo "$_pay_resp" | jq -re '.access_token // .token // empty' 2>/dev/null)
  if [[ -n "$_token" ]]; then
