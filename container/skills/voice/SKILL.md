...
- `voiceId` defaults to `21m00Tcm4TlvDq8ikWAM` (Rachel, ElevenLabs default). Only override if the user has named a different voice.
...
3. **Stream → decode → play** in one pipeline (no temp file):
   ...
   curl -sS -N \
     ...
     -d '{"text":"<your text here>","voiceId":"21m00Tcm4TlvDq8ikWAM","outputFormat":"mp3_44100_128"}' \
...
BODY=$(jq -nc \
  --arg text "$REPLY_TEXT" \
  --arg voice "21m00Tcm4TlvDq8ikWAM" \
...
|---------|---------|-------|
| Voice ID | `21m00Tcm4TlvDq8ikWAM` | Rachel (ElevenLabs default). |
